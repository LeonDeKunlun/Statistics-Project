...
) %>% purrr::iwalk(\(x, idx) ggplot2::update_geom_defaults(geom = idx, new = x))
```
...
create_variable_plot <- function(x, y) {
  gas_demand_wide %>%
    ggplot2::ggplot(
      mapping = ggplot2::aes(x = {{ x }}, y = {{ y }})) +
    ggplot2::labs(title = rlang::englue("{{ y }} vs {{ x }}"))
}
```

```{r}
create_variable_plot(`Gas Day`, `Gas Demand`) +
  ggplot2::geom_line()
```
